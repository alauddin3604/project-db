...
if (isset($_SESSION['workload_id']) && isset($_SESSION['subject_code']))
{
	...
	$sql = 'SELECT s.student_name, ss.student_id, m.mark
			...
			LEFT JOIN mark_truefalse m ON (m.workload_id = ss.workload_id) AND (m.student_id = ss.student_id)
	...
	// Get students who pass or fail
	$sql = 'SELECT COUNT(*) FROM quiz_truefalse WHERE workload_id = ?';
	$stmt = $conn->prepare($sql);
	$stmt->bind_param('i', $workload_id);
	$stmt->execute();
	$result = $stmt->get_result();
...
<body>
	<div class="w3-container">
		...
		$numPass = 0;
		$numFail = 0;
		$sql = 'SELECT mark FROM mark_truefalse WHERE workload_id = ?';
		$stmt = $conn->prepare($sql);
		$stmt->bind_param('i', $workload_id);
		if ($stmt->execute())
		{
